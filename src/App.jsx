...
const GLOBAL_STYLES = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  body { background: #080e1a; font-family: 'DM Sans', sans-serif; color: #e2eaf4; overflow-x: hidden; }
  ...
  .topic-item { transition: background 0.15s, border-left-color 0.15s; }
  .topic-item:hover { background: rgba(0,180,216,0.07) !important; }

  .opt-btn { transition: all 0.15s; }
  .opt-btn:hover:not(:disabled) { background: rgba(0,180,216,0.12) !important; border-color: rgba(0,180,216,0.5) !important; }

  .card-lift { transition: transform 0.2s, box-shadow 0.2s; }
  .card-lift:hover { transform: translateY(-3px); box-shadow: 0 12px 36px rgba(0,180,216,0.18); }
  ...
  @media (max-width: 768px) {
    .desktop-sidebar { position: fixed !important; left: -280px !important; top: 0 !important; height: 100vh !important; z-index: 50 !important; transition: left 0.3s ease !important; width: 280px !important; }
    .desktop-sidebar.open { left: 0 !important; }
    .learn-content { margin-left: 0 !important; }
  }
...
const SECTIONS = [
  {
    id: "fundamentals", icon: "🏢", color: "#00B4D8",
...
Beyond hardware redundancy, resilience requires: diverse cable routes entering the building from different directions, geographic separation of primary and DR sites, tested failover procedures, documented Emergency Operating Procedures (EOPs), regular disaster recovery exercises, and real-time monitoring with automated alerting.`,
  ...
  {
    id: "compliance", icon: "📋", color: "#F0A500",
...
A third-party audit and certification programme confirming that a facility genuinely meets the claimed Tier level — not just in design (Design Documents review), but in construction (Constructed Facility review) and ongoing operations (Management & Operations review). The "gold standard" of data centre credibility.`,
  ...
  {
    id: "infrastructure", icon: "⚡", color: "#22C55E",
...
Removing equipment requires: removing it from the asset register, secure data erasure (NIST SP 800-88 guidelines — overwrite, purge, or physically destroy storage media), environmentally compliant disposal under WEEE regulations, and updating all documentation. Improper decommissioning of storage media has led to major data breaches.`,
  ...
  {
    id: "maintenance", icon: "🔧", color: "#8B5CF6",
...
- **CUE (Carbon Usage Effectiveness):** CO₂ equivalent emissions ÷ IT energy — measures carbon intensity`,
  ...
  {
    id: "cooling", icon: "❄️", color: "#0EA5E9",
...
export default function App() {
  ...
  if (view === "home") {
    return (
      <div style={{ minHeight: "100vh", background: "#080e1a" }}>
        ...
        {/* Nav */}
        <nav style={{ background: "#0a1220", borderBottom: "1px solid #1a2840", padding: "0 20px", height: 56, display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, zIndex: 30 }}>
          <span style={{ fontFamily: "'Playfair Display', serif", fontSize: 18, fontWeight: 700, color: "#e2eaf4" }}>DC Training</span>
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={() => setView("quiz")} style={{ padding: "7px 14px", background: "transparent", color: "#94a3b8", border: "1px solid #1a2840", borderRadius: 6, fontSize: 13 }}>Quizzes</button>
            <button onClick={() => { setMockAnswers({}); setMockSubmitted(false); setView("mock"); }} style={{ padding: "7px 14px", background: "#00B4D8", color: "#080e1a", border: "none", borderRadius: 6, fontSize: 13, fontWeight: 600 }}>Mock Test</button>
          </div>
        </nav>

        {/* Hero */}
        <div style={{ padding: "clamp(32px, 6vw, 72px) clamp(16px, 5vw, 60px)", maxWidth: 1100, margin: "0 auto" }}>
          <div className="fade-in" style={{ marginBottom: 48 }}>
            <div style={{ display: "inline-block", padding: "4px 14px", background: "rgba(0,180,216,0.1)", border: "1px solid rgba(0,180,216,0.25)", borderRadius: 20, fontSize: 12, color: "#00B4D8", letterSpacing: 2, textTransform: "uppercase", fontFamily: "'DM Sans', sans-serif", fontWeight: 600, marginBottom: 20 }}>
              7 Modules · 23 Topics · Quizzes & Mock Test
            </div>
            <h1 style={{ fontFamily: "'Playfair Display', serif", fontSize: "clamp(32px, 6vw, 60px)", fontWeight: 900, lineHeight: 1.1, color: "#e2eaf4", marginBottom: 20 }}>
              Data Centre<br /><span style={{ color: "#00B4D8" }}>Fundamentals</span>
            </h1>
            <p style={{ fontSize: "clamp(14px, 2vw, 17px)", color: "#64748b", maxWidth: 540, lineHeight: 1.75 }}>
              Master data centre concepts with detailed explanations, real-world examples, topic quizzes, and a 30-question final mock exam.
            </p>
            <div style={{ display: "flex", gap: 12, marginTop: 28, flexWrap: "wrap" }}>
              <button className="card-lift" onClick={() => goToLearn(SECTIONS[0].id)} style={{ padding: "13px 28px", background: "#00B4D8", color: "#080e1a", borderRadius: 8, fontSize: 15, fontWeight: 700, border: "none" }}>
                Start Learning →
              </button>
              <button className="card-lift" onClick={() => { setMockAnswers({}); setMockSubmitted(false); setView("mock"); }} style={{ padding: "13px 28px", background: "rgba(240,165,0,0.12)", color: "#F0A500", border: "1px solid rgba(240,165,0,0.3)", borderRadius: 8, fontSize: 15, fontWeight: 600 }}>
                Mock Test (30 Qs)
              </button>
            </div>
          </div>
            ...
            {SECTIONS.map((s, i) => {
              ...
              return (
                <div key={s.id} className="card-lift fade-in" onClick={() => goToLearn(s.id)}
                  style={{ animationDelay: `${i * 0.05}s`, background: "#0d1525", border: `1px solid ${s.color}25`, borderRadius: 12, padding: "20px 20px 16px", cursor: "pointer", position: "relative", overflow: "hidden" }}>
                  <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: 3, background: s.color }} />
                  <div style={{ fontSize: 28, marginBottom: 10 }}>{s.icon}</div>
                  <h3 style={{ fontFamily: "'Playfair Display', serif", fontSize: 16, color: "#e2eaf4", marginBottom: 6, lineHeight: 1.3 }}>{s.title}</h3>
                  <p style={{ fontSize: 12, color: "#475569", marginBottom: 14 }}>{s.topics.length} topics · {(QUIZ_QUESTIONS[s.id] || []).length} questions</p>
                  ...
                  <div style={{ fontSize: 11, color: s.color, marginTop: 5, fontWeight: 600 }}>{pct}% quiz done</div>
                </div>
              );
            })}
          </div>
  ...
  if (view === "learn") {
    return (
      <div style={{ minHeight: "100vh", background: "#080e1a", display: "flex", flexDirection: "column" }}>
        ...
        {/* Top bar */}
        <div style={{ background: "#0a1220", borderBottom: "1px solid #1a2840", height: 52, display: "flex", alignItems: "center", padding: "0 16px", gap: 12, position: "sticky", top: 0, zIndex: 35, flexShrink: 0 }}>
          {/* Burger */}
          <button className="burger-btn" onClick={() => setSidebarOpen(o => !o)}
            style={{ width: 36, height: 36, background: "none", display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", gap: 5, padding: 4, borderRadius: 6 }}>
            <div className="burger-line" style={{ width: 20, height: 2, background: sidebarOpen ? "transparent" : "#94a3b8", transform: sidebarOpen ? "rotate(45deg) translate(5px, 5px)" : "none" }} />
            <div className="burger-line" style={{ width: 20, height: 2, background: "#94a3b8", opacity: sidebarOpen ? 0 : 1 }} />
            <div className="burger-line" style={{ width: 20, height: 2, background: sidebarOpen ? "transparent" : "#94a3b8", transform: sidebarOpen ? "rotate(-45deg) translate(5px, -5px)" : "none" }} />
          </button>

          <button onClick={() => setView("home")} style={{ background: "none", color: "#64748b", fontSize: 13, padding: "4px 8px", borderRadius: 4, border: "none" }}>← Home</button>

          <div style={{ flex: 1, display: "flex", alignItems: "center", gap: 6, overflow: "hidden" }}>
            <span style={{ fontSize: 16 }}>{activeSection.icon}</span>
            <span style={{ fontFamily: "'Playfair Display', serif", fontSize: 14, color: "#94a3b8", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{activeSection.title}</span>
          </div>

          <button onClick={() => { setQuizSectionId(activeSection.id); setQuizAnswers({}); setQuizSubmitted(false); setView("quiz"); }}
            style={{ padding: "6px 14px", background: `${activeSection.color}18`, color: activeSection.color, border: `1px solid ${activeSection.color}35`, borderRadius: 6, fontSize: 12, fontWeight: 600, whiteSpace: "nowrap" }}>
            Quiz →
          </button>
        </div>
        ...
        <div style={{ display: "flex", flex: 1, overflow: "hidden", position: "relative" }}>
          ...
          <div className={`desktop-sidebar ${sidebarOpen ? "open" : ""}`}
            style={{ width: 260, background: "#0a1220", borderRight: "1px solid #1a2840", overflowY: "auto", flexShrink: 0 }}>

            {SECTIONS.map((sec) => (
              <div key={sec.id}>
                {/* Section header */}
                <div style={{ padding: "14px 16px 8px", borderTop: "1px solid #1a2840", display: "flex", alignItems: "center", gap: 8 }}>
                  <span style={{ fontSize: 16 }}>{sec.icon}</span>
                  <span style={{ fontSize: 12, fontWeight: 600, color: sec.id === activeSectionId ? sec.color : "#475569", letterSpacing: 0.3 }}>{sec.title}</span>
                </div>
                ...
                {sec.topics.map((t, ti) => {
                  ...
                  return (
                    <button key={ti} className="topic-item"
                      onClick={() => { goToLearn(sec.id, ti); }}
                      style={{ width: "100%", textAlign: "left", padding: "9px 16px 9px 36px", background: isActive ? `${sec.color}15` : "transparent", borderLeft: `3px solid ${isActive ? sec.color : "transparent"}`, border: "none", cursor: "pointer", display: "block" }}>
                      <span style={{ fontSize: 13, color: isActive ? "#e2eaf4" : "#475569", lineHeight: 1.4, display: "block" }}>{t.title}</span>
                    </button>
                  );
                })}
              </div>
          ...
          <div ref={contentRef} style={{ flex: 1, overflowY: "auto", padding: "clamp(16px, 4vw, 40px)" }}>
            <div style={{ maxWidth: 780, margin: "0 auto" }} className="fade-in">
              ...
              <div style={{ display: "flex", alignItems: "flex-start", gap: 12, marginBottom: 28 }}>
                <div style={{ width: 4, minHeight: 44, background: activeSection.color, borderRadius: 2, flexShrink: 0, marginTop: 4 }} />
                <div>
                  <div style={{ fontSize: 11, color: activeSection.color, letterSpacing: 3, textTransform: "uppercase", fontWeight: 700, marginBottom: 6 }}>
                    {activeSection.icon} {activeSection.title}
                  </div>
                  <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: "clamp(20px, 4vw, 28px)", color: "#e2eaf4", lineHeight: 1.2 }}>{activeTopic.title}</h2>
                </div>
              </div>

              {/* Content */}
              <div style={{ background: "#0d1525", borderRadius: 12, padding: "clamp(16px, 4vw, 28px)", marginBottom: 18, border: "1px solid #1a2840", lineHeight: 1.85, fontSize: "clamp(13px, 2vw, 15px)", color: "#94a3b8" }}>
                {activeTopic.content.split("\n\n").map((para, i, arr) => (
                  <p key={i} style={{ marginBottom: i < arr.length - 1 ? 18 : 0 }}
                    dangerouslySetInnerHTML={{ __html: para.replace(/\*\*(.*?)\*\*/g, `<strong style="color:#c8d9e8;font-weight:600">$1</strong>`) }} />
                ))}
              </div>

              {/* Example */}
              <div style={{ background: `linear-gradient(135deg, ${activeSection.color}10, ${activeSection.color}05)`, border: `1px solid ${activeSection.color}28`, borderRadius: 12, padding: "clamp(14px, 3vw, 24px)", marginBottom: 18 }}>
                <div style={{ fontSize: 10, color: activeSection.color, letterSpacing: 3, textTransform: "uppercase", fontWeight: 700, marginBottom: 10 }}>Real-World Example</div>
                <p style={{ fontSize: "clamp(13px, 2vw, 15px)", color: "#94a3b8", lineHeight: 1.85 }}
                  dangerouslySetInnerHTML={{ __html: activeTopic.example.replace(/\*\*(.*?)\*\*/g, `<strong style="color:${activeSection.color};font-weight:600">$1</strong>`) }} />
              </div>

              {/* Key Points */}
              <div style={{ background: "#08111C", borderRadius: 12, padding: "clamp(14px, 3vw, 24px)", border: "1px solid #1a2840", marginBottom: 32 }}>
                <div style={{ fontSize: 10, color: "#475569", letterSpacing: 3, textTransform: "uppercase", fontWeight: 700, marginBottom: 14 }}>Key Points</div>
                <div style={{ display: "flex", flexDirection: "column", gap: 9 }}>
                  {activeTopic.keyPoints.map((kp, i) => (
                    <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: 10 }}>
                      ...
                      <span style={{ fontSize: "clamp(12px, 1.8vw, 14px)", color: "#64748b", lineHeight: 1.6 }}
                        dangerouslySetInnerHTML={{ __html: kp.replace(/\*\*(.*?)\*\*/g, `<strong style="color:#94a3b8">$1</strong>`) }} />
                    </div>
                  ))}
                </div>
              </div>
              ...
              {/* Navigation */}
              <div style={{ display: "flex", justifyContent: "space-between", gap: 12, paddingTop: 20, borderTop: "1px solid #1a2840" }}>
                <button onClick={() => {
                  if (activeTopicIdx > 0) { goToLearn(activeSectionId, activeTopicIdx - 1); }
                  else {
                    const idx = SECTIONS.findIndex(s => s.id === activeSectionId);
                    if (idx > 0) goToLearn(SECTIONS[idx - 1].id, SECTIONS[idx - 1].topics.length - 1);
                  }
                }} style={{ padding: "10px 18px", background: "#0d1525", color: "#64748b", borderRadius: 8, border: "1px solid #1a2840", fontSize: 14 }}>← Prev</button>

                <button onClick={() => {
                  if (activeTopicIdx < activeSection.topics.length - 1) { goToLearn(activeSectionId, activeTopicIdx + 1); }
                  else {
                    const idx = SECTIONS.findIndex(s => s.id === activeSectionId);
                    if (idx < SECTIONS.length - 1) goToLearn(SECTIONS[idx + 1].id, 0);
                    else { setQuizSectionId(activeSectionId); setQuizAnswers({}); setQuizSubmitted(false); setView("quiz"); }
                  }
                }} style={{ padding: "10px 20px", background: activeSection.color, color: "#080e1a", borderRadius: 8, border: "none", fontSize: 14, fontWeight: 700 }}>
                  {activeTopicIdx < activeSection.topics.length - 1 ? "Next →" : "Take Quiz →"}
                </button>
              </div>
            </div>
          </div>
  ...
  if (view === "quiz" && !quizSectionId) {
    return (
      <div style={{ minHeight: "100vh", background: "#080e1a", padding: "0 0 40px" }}>
        <style>{GLOBAL_STYLES}</style>
        <nav style={{ background: "#0a1220", borderBottom: "1px solid #1a2840", padding: "0 20px", height: 52, display: "flex", alignItems: "center", gap: 12, position: "sticky", top: 0, zIndex: 30 }}>
          <button onClick={() => setView("home")} style={{ background: "none", color: "#64748b", fontSize: 13, border: "none" }}>← Home</button>
          <span style={{ fontFamily: "'Playfair Display', serif", fontSize: 16, color: "#e2eaf4" }}>Topic Quizzes</span>
        </nav>
        <div style={{ maxWidth: 900, margin: "0 auto", padding: "clamp(20px,4vw,48px) clamp(16px,4vw,32px)" }}>
          <p style={{ fontSize: 15, color: "#475569", marginBottom: 32 }}>Select a module to test your knowledge — 5 questions per module with full explanations.</p>
            ...
            {SECTIONS.map((s, i) => {
              ...
              return (
                <div key={s.id} className="card-lift fade-in" onClick={() => { setQuizSectionId(s.id); setQuizAnswers({}); setQuizSubmitted(false); }}
                  style={{ animationDelay: `${i * 0.04}s`, background: "#0d1525", border: `1px solid ${s.color}25`, borderRadius: 12, padding: 20, cursor: "pointer" }}>
                  <div style={{ fontSize: 28, marginBottom: 8 }}>{s.icon}</div>
                  <h3 style={{ fontFamily: "'Playfair Display', serif", fontSize: 15, color: "#e2eaf4", marginBottom: 6 }}>{s.title}</h3>
                  <p style={{ fontSize: 12, color: "#475569", marginBottom: 12 }}>{(QUIZ_QUESTIONS[s.id] || []).length} questions · {done} completed</p>
          ...
          <div style={{ textAlign: "center", marginTop: 40 }}>
            <button className="card-lift" onClick={() => { setMockAnswers({}); setMockSubmitted(false); setView("mock"); }}
              style={{ padding: "13px 36px", background: "#F0A500", color: "#080e1a", borderRadius: 8, border: "none", fontSize: 15, fontWeight: 700 }}>
              Take Full 30-Question Mock Test →
            </button>
          </div>
        </div>
      </div>
  ...
  if (view === "quiz" && quizSectionId && quizSection) {
    ...
    return (
      <div style={{ minHeight: "100vh", background: "#080e1a", paddingBottom: 40 }}>
        <style>{GLOBAL_STYLES}</style>
        <nav style={{ background: "#0a1220", borderBottom: "1px solid #1a2840", padding: "0 16px", height: 52, display: "flex", alignItems: "center", gap: 10, position: "sticky", top: 0, zIndex: 30 }}>
          <button onClick={() => setQuizSectionId(null)} style={{ background: "none", color: "#64748b", fontSize: 13, border: "none" }}>← Quizzes</button>
          <span style={{ fontSize: 14 }}>{quizSection.icon}</span>
          <span style={{ fontFamily: "'Playfair Display', serif", fontSize: 14, color: "#94a3b8" }}>{quizSection.title}</span>
          <span style={{ marginLeft: "auto", fontSize: 13, color: "#475569" }}>{answered}/{quizQs.length} answered</span>
        </nav>

        <div style={{ maxWidth: 720, margin: "0 auto", padding: "clamp(16px,4vw,36px) clamp(12px,4vw,24px)" }}>

          {/* Questions */}
          {quizQs.map((q, qi) => {
            const isCorrect = quizAnswers[qi] === q.answer;
            return (
              <div key={qi} className="fade-in" style={{ animationDelay: `${qi * 0.04}s`, background: "#0d1525", borderRadius: 12, padding: "clamp(14px,3vw,22px)", marginBottom: 16,
                border: quizSubmitted ? `1px solid ${isCorrect ? "#22C55E40" : "#EF444440"}` : "1px solid #1a2840" }}>
                <div style={{ display: "flex", gap: 10, marginBottom: 14 }}>
                  <div style={{ width: 26, height: 26, borderRadius: "50%", background: `${quizSection.color}20`, color: quizSection.color, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 700, flexShrink: 0 }}>{qi + 1}</div>
                  <p style={{ fontSize: "clamp(13px,2vw,15px)", color: "#c8d9e8", lineHeight: 1.55 }}>{q.q}</p>
                </div>
                <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
                  {q.options.map((opt, oi) => {
                    let bg = "#08111C", border = "#1a2840", col = "#64748b";
                    if (!quizSubmitted && quizAnswers[qi] === oi) { bg = `${quizSection.color}18`; border = quizSection.color; col = "#e2eaf4"; }
                    if (quizSubmitted) {
                      if (oi === q.answer) { bg = "rgba(34,197,94,0.1)"; border = "#22C55E"; col = "#e2eaf4"; }
                      else if (quizAnswers[qi] === oi) { bg = "rgba(239,68,68,0.1)"; border = "#EF4444"; col = "#e2eaf4"; }
                      else { col = "#334155"; }
                    }
                    return (
                      <button key={oi} className="opt-btn" disabled={quizSubmitted}
                        ...
                        style={{ padding: "10px 14px", background: bg, border: `1px solid ${border}`, borderRadius: 8, color: col, fontSize: "clamp(12px,1.8vw,14px)", display: "flex", alignItems: "center", gap: 9, textAlign: "left" }}>
                        <span style={{ width: 20, height: 20, borderRadius: "50%", border: `1px solid ${border}`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10, flexShrink: 0 }}>
                ...
                {quizSubmitted && (
                  <div style={{ marginTop: 12, padding: "12px 14px", background: "rgba(0,180,216,0.07)", borderRadius: 8, border: "1px solid rgba(0,180,216,0.18)" }}>
                    <span style={{ fontSize: "clamp(12px,1.8vw,13px)", color: "#64748b" }}>
          ...
          {/* Submit */}
          {!quizSubmitted && (
            <button disabled={answered < quizQs.length}
              onClick={() => { setQuizSubmitted(true); if (contentRef.current) contentRef.current.scrollTop = 0; }}
              style={{ width: "100%", padding: "14px", background: answered === quizQs.length ? quizSection.color : "#1a2840", color: answered === quizQs.length ? "#080e1a" : "#334155", borderRadius: 10, border: "none", fontSize: 15, fontWeight: 700, cursor: answered === quizQs.length ? "pointer" : "not-allowed", marginTop: 8 }}>
              {answered < quizQs.length ? `Answer all questions (${answered}/${quizQs.length})` : "Submit Answers →"}
            </button>
          )}

          {/* Results — BOTTOM */}
          {quizSubmitted && (
            <div className="fade-in" style={{ marginTop: 24, background: pct >= 80 ? "rgba(34,197,94,0.08)" : pct >= 60 ? "rgba(240,165,0,0.08)" : "rgba(239,68,68,0.08)",
              border: `1px solid ${pct >= 80 ? "#22C55E" : pct >= 60 ? "#F0A500" : "#EF4444"}40`, borderRadius: 14, padding: "clamp(18px,4vw,28px)", textAlign: "center" }}>
              <div style={{ fontSize: 40, marginBottom: 8 }}>{pct >= 80 ? "🎉" : pct >= 60 ? "👍" : "📖"}</div>
              <div style={{ fontFamily: "'Playfair Display', serif", fontSize: "clamp(28px,5vw,36px)", color: "#e2eaf4", marginBottom: 4 }}>{score} / {quizQs.length}</div>
              <div style={{ fontSize: 15, color: pct >= 80 ? "#22C55E" : pct >= 60 ? "#F0A500" : "#EF4444", fontWeight: 700, marginBottom: 6 }}>{pct}%</div>
              <div style={{ fontSize: 14, color: "#64748b", marginBottom: 20 }}>
                {pct >= 80 ? "Excellent — you've mastered this section!" : pct >= 60 ? "Good effort — review the explanations above." : "Keep studying — revisit the topics and retry."}
              </div>
              <div style={{ display: "flex", gap: 10, justifyContent: "center", flexWrap: "wrap" }}>
                <button onClick={() => { setQuizAnswers({}); setQuizSubmitted(false); window.scrollTo(0, 0); }}
                  style={{ padding: "10px 22px", background: quizSection.color, color: "#080e1a", borderRadius: 8, border: "none", fontWeight: 700, fontSize: 14, cursor: "pointer" }}>
                  Retry Quiz
                </button>
                <button onClick={() => goToLearn(quizSectionId)}
                  style={{ padding: "10px 22px", background: "#1a2840", color: "#94a3b8", borderRadius: 8, border: "none", fontSize: 14, cursor: "pointer" }}>
                  Review Topics
                </button>
  ...
  if (view === "mock") {
    ...
    return (
      <div style={{ minHeight: "100vh", background: "#080e1a", paddingBottom: 120 }}>
        <style>{GLOBAL_STYLES}</style>
        <nav style={{ background: "#0a1220", borderBottom: "1px solid #1a2840", padding: "0 16px", height: 52, display: "flex", alignItems: "center", gap: 10, position: "sticky", top: 0, zIndex: 30 }}>
          <button onClick={() => setView("home")} style={{ background: "none", color: "#64748b", fontSize: 13, border: "none" }}>← Home</button>
          <span style={{ fontFamily: "'Playfair Display', serif", fontSize: 16, color: "#e2eaf4" }}>Final Mock Exam</span>
          <span style={{ marginLeft: "auto", fontSize: 12, color: "#475569" }}>Pass: 70% (21/30)</span>
        </nav>

        <div style={{ maxWidth: 780, margin: "0 auto", padding: "clamp(16px,4vw,36px) clamp(12px,4vw,24px)" }}>

          {/* Questions */}
          {MOCK_TEST.map((q, qi) => {
            const sec = SECTIONS.find(s => s.id === q.section);
            const col = sec ? sec.color : "#00B4D8";
            const isCorrect = mockAnswers[qi] === q.answer;
            return (
              <div key={qi} style={{ background: "#0d1525", borderRadius: 12, padding: "clamp(14px,3vw,20px)", marginBottom: 14,
                border: mockSubmitted ? `1px solid ${isCorrect ? "#22C55E35" : "#EF444435"}` : "1px solid #1a2840" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
                  <span style={{ fontSize: 10, color: col, letterSpacing: 2, textTransform: "uppercase", fontWeight: 700 }}>{sec?.icon} {sec?.title.replace("Data Centre ", "").replace("The Physical ", "")}</span>
                  {mockSubmitted && <span style={{ marginLeft: "auto" }}>{isCorrect ? "✅" : "❌"}</span>}
                </div>
                <div style={{ display: "flex", gap: 10, marginBottom: 12 }}>
                  <div style={{ width: 24, height: 24, borderRadius: "50%", background: `${col}18`, color: col, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 700, flexShrink: 0 }}>{qi + 1}</div>
                  <p style={{ fontSize: "clamp(13px,2vw,15px)", color: "#c8d9e8", lineHeight: 1.55 }}>{q.q}</p>
                </div>
                <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                  {q.options.map((opt, oi) => {
                    let bg = "#08111C", border = "#1a2840", color = "#64748b";
                    if (!mockSubmitted && mockAnswers[qi] === oi) { bg = `${col}18`; border = col; color = "#e2eaf4"; }
                    if (mockSubmitted) {
                      if (oi === q.answer) { bg = "rgba(34,197,94,0.1)"; border = "#22C55E"; color = "#e2eaf4"; }
                      else if (mockAnswers[qi] === oi) { bg = "rgba(239,68,68,0.1)"; border = "#EF4444"; color = "#e2eaf4"; }
                      else { color = "#2d3f55"; }
                    }
                    return (
                      <button key={oi} className="opt-btn" disabled={mockSubmitted}
                        onClick={() => { if (!mockSubmitted) setMockAnswers(a => ({ ...a, [qi]: oi })); }}
                        style={{ padding: "9px 12px", background: bg, border: `1px solid ${border}`, borderRadius: 7, color, fontSize: "clamp(12px,1.8vw,13px)", display: "flex", alignItems: "center", gap: 8, textAlign: "left" }}>
                        <span style={{ width: 18, height: 18, borderRadius: "50%", border: `1px solid ${border}`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 9, flexShrink: 0 }}>
          ...
          {/* Submit */}
          {!mockSubmitted && (
            <button disabled={answered < MOCK_TEST.length}
              onClick={() => { setMockSubmitted(true); window.scrollTo(0, 0); }}
              style={{ width: "100%", padding: "15px", background: answered === MOCK_TEST.length ? "#F0A500" : "#1a2840", color: answered === MOCK_TEST.length ? "#080e1a" : "#334155", borderRadius: 10, border: "none", fontSize: 15, fontWeight: 700, cursor: answered === MOCK_TEST.length ? "pointer" : "not-allowed", marginTop: 8 }}>
          ...
          {/* Results — BOTTOM */}
          {mockSubmitted && (
            <div className="fade-in" style={{ marginTop: 28, background: pct >= 70 ? "rgba(34,197,94,0.07)" : "rgba(239,68,68,0.07)", border: `1px solid ${pct >= 70 ? "#22C55E" : "#EF4444"}35`, borderRadius: 14, padding: "clamp(18px,4vw,32px)" }}>
              <div style={{ display: "flex", gap: 20, alignItems: "center", flexWrap: "wrap", marginBottom: 24 }}>
                <div style={{ textAlign: "center" }}>
                  <div style={{ fontFamily: "'Playfair Display', serif", fontSize: "clamp(44px,8vw,60px)", fontWeight: 900, color: pct >= 70 ? "#22C55E" : "#EF4444", lineHeight: 1 }}>{score}</div>
                  <div style={{ fontSize: 12, color: "#475569" }}>out of 30</div>
                </div>
                <div>
                  <div style={{ fontFamily: "'Playfair Display', serif", fontSize: "clamp(24px,4vw,32px)", color: "#e2eaf4", marginBottom: 4 }}>{pct}%</div>
                  <div style={{ fontSize: 18, fontWeight: 700, color: pct >= 70 ? "#22C55E" : "#EF4444" }}>{pct >= 70 ? "✓ PASS" : "✗ FAIL"}</div>
                  <div style={{ fontSize: 13, color: "#475569", marginTop: 4 }}>Pass mark: 70% (21/30)</div>
                </div>
              </div>

              {/* Section breakdown */}
              <div style={{ marginBottom: 22 }}>
                <div style={{ fontSize: 11, color: "#334155", letterSpacing: 2, textTransform: "uppercase", marginBottom: 12 }}>Score by Module</div>
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(180px, 1fr))", gap: 8 }}>
                  {sectionBreakdown.map(s => (
                    <div key={s.id} style={{ background: "#08111C", borderRadius: 8, padding: "10px 12px", border: `1px solid ${s.color}25` }}>
                      <div style={{ fontSize: 11, color: s.color, marginBottom: 4 }}>{s.icon} {s.title.replace("Data Centre ", "")}</div>
                      <div style={{ fontSize: 18, fontWeight: 700, color: "#e2eaf4" }}>{s.correct}/{s.total}</div>
                      <div style={{ height: 3, background: "#1a2840", borderRadius: 2, marginTop: 6 }}>
              ...
              <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                <button onClick={() => { setMockAnswers({}); setMockSubmitted(false); window.scrollTo(0, 0); }}
                  style={{ padding: "10px 24px", background: "#F0A500", color: "#080e1a", borderRadius: 8, border: "none", fontWeight: 700, fontSize: 14, cursor: "pointer" }}>
                  Retake Test
                </button>
                <button onClick={() => goToLearn(SECTIONS[0].id)}
                  style={{ padding: "10px 24px", background: "#1a2840", color: "#94a3b8", borderRadius: 8, border: "none", fontSize: 14, cursor: "pointer" }}>
                  Review Learning
                </button>
              </div>
            </div>
        ...
        {/* Sticky progress bar */}
        {!mockSubmitted && (
          <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, background: "#0a1220", borderTop: "1px solid #1a2840", padding: "12px 20px", zIndex: 30 }}>
            <div style={{ maxWidth: 780, margin: "0 auto", display: "flex", alignItems: "center", gap: 14 }}>
              <div style={{ flex: 1, height: 4, background: "#1a2840", borderRadius: 2 }}>
                <div style={{ height: "100%", width: `${(answered / MOCK_TEST.length) * 100}%`, background: "#F0A500", borderRadius: 2, transition: "width 0.3s" }} />
              </div>
              <span style={{ fontSize: 13, color: "#475569", whiteSpace: "nowrap" }}>{answered}/30</span>
            </div>
          </div>
        )}
      </div>
